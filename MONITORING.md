...
이 기능은 GitHub Actions에서 매시간 7분에 실행되어 서초사옥의 다음 날짜/시간대 예약 상태를 확인합니다. 자동 예약이나 신청은 하지 않습니다.
...
- 각 날짜/시간대별로 `예약마감 → 예약가능`으로 바뀔 때 한 번만 알림
- 같은 예약가능 상태에서는 중복 알림하지 않음
- 한 번에 여러 날짜/시간대가 동시에 열리면 텔레그램 메시지 하나에 모아서 알림
